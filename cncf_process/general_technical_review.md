...
- When installing Karmada, users can either customize certificates or use the ones automatically generated by Karmada, with configurable validity periods.
...
Karmada defines SLOs and SLIs through a reliability-engineering workflow documented in its [Reliability Engineering Guide](https://karmada.io/docs/administrator/reliability/guide). The project starts from the critical user journey of multi-cluster workload delivery, decomposes it into control-plane stages such as API access, policy matching, scheduling, work generation, workload propagation, and cluster health synchronization, and then assigns measurable SLIs to each stage.

The selected SLIs focus on whether each stage succeeds and how long it takes, using availability and latency as the primary reliability dimensions. These indicators are derived from Karmada's [Prometheus-compatible metrics](https://karmada.io/docs/next/reference/instrumentation/metrics), so adopters can monitor them with standard cloud-native observability tooling.

Karmada also documents how to operationalize these definitions: use Sloth to express SLOs, generate Prometheus recording and alerting rules, load those rules into Prometheus, visualize them in Grafana dashboards, and connect alerts to runbooks. The documented objectives are intended as recommended defaults that adopters can tune based on their own scale, workload criticality, and error-budget policy.
...
Karmada documents its current SLOs in the [Reliability Engineering Guide](https://karmada.io/docs/administrator/reliability/guide). The SLOs are organized around the major stages of the Karmada control-plane workflow, including API server access, policy application, scheduling, resource propagation, and cluster health synchronization.

The SLO model includes the related SLIs, error-budget model, alerting strategy, Prometheus/Sloth setup flow, Grafana dashboards, and runbooks for day-2 operations.

In addition, the community's 2022 [large-scale test report](https://karmada.io/blog/2022/10/26/test-report#slisslos) remains a useful historical reference for earlier large-scale SLI/SLO and performance validation.
