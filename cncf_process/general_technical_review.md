...
- **Feature Gathering:**
  - Feature requests and proposals are collected from:
    ...
    - Input from the [Karmada Adopter Group](https://github.com/karmada-io/community/tree/main/adopter-group)
...
- **AI Infrastructure Platform Teams**:
  - Use Karmada to build and operate multi-cluster AI infrastructure for large-scale training and inference workloads.
...
![Karmada Instance HA](images/architecture-overview.png)
...
Karmada documents certificate usage and rotation under its [Certificate Framework](https://karmada.io/docs/administrator/security/cert-framework) and [Certificate Rotation](https://karmada.io/docs/administrator/security/certificate-rotation/overview) guides, giving operators explicit guidance on certificate layout, rotation workflows, and operational caveats.

- When installing Karmada, users can either customize certificates or use the certificates automatically generated by Karmada. When generating certificates through Karmada, users can configure certificate validity periods.
- Karmada now includes built-in certificate rotation support for `karmada-agent`, and documents how that workflow is enabled and operated.
- Other certificates are still rotated through administrator-controlled procedures using Kubernetes Secrets/ConfigMaps and the documented certificate topology.
...
Karmada supports multiple installation methods. Among them, `Helm` and `karmada-operator` support both rollout and rollback.

- **Helm**: rollback can be performed by using the `helm rollback` command to restore a previous release revision.
- **karmada-operator**: rollout is managed through updates to the `Karmada` custom resource (CR) spec, and rollback can be achieved by reverting the `Karmada` CR spec to its previous desired state.
...
Traffic will not be switched to the new unless the newer revision is ready to accept traffic. Hence, the already running workloads will not be affected in any case if rollout/rollback fails.

#### Describe any specific metrics that should inform a rollback.

A rollback is usually considered when the actual runtime state does not match the expected state. For Karmada, some signals that may inform a rollback decision include:

- the readiness status of Karmada components remaining `False` for a sustained period;
- for Helm-based installations, an abnormal or failed release status reported by `helm status`;
- for `karmada-operator`-based installations, the Ready condition of the `Karmada` custom resource (CR) remaining `False`.

#### Explain how upgrades and rollbacks were tested and how the upgrade->downgrade->upgrade path was tested.

Upgrade->downgrade->upgrade testing is not covered. 

For rollback tests:
- If installed via Helm, use `helm rollback` to perform the rollback test.
- If installed via karmada-operator, update the Karmada CR (Custom Resource) to a previous version to conduct the rollback test. Refer to https://github.com/karmada-io/karmada/blob/master/operator/README.md#upgrade-a-karmada-instance.
...
Karmada now documents reliability targets in its [Reliability Engineering Guide](https://karmada.io/docs/administrator/reliability/guide). Instead of defining only a few coarse-grained latency indicators, the project now maps SLOs to the end-to-end resource propagation pipeline - policy matching, scheduling, work creation, work execution, and cluster health synchronization - and pairs each critical stage with concrete SLIs for availability and/or latency.

In addition, the community's 2022 [test report and SLI/SLO discussion](https://karmada.io/blog/2022/10/26/test-report#slisslos) remains useful background for Karmada's large-scale performance and reliability work.

These SLOs are presented as recommended starting points for adopters and are backed by Karmada's [Prometheus-compatible metrics](https://karmada.io/docs/next/reference/instrumentation/metrics), generated alert rules, and runbooks. The guide is not only descriptive; it also helps adopters set up their own SLO practice by showing how to define SLOs with Sloth, generate Prometheus recording and alerting rules, load those rules into Prometheus, and use Grafana dashboards and runbooks for day-2 operations. The current documented SLO set includes:

| Category                                   | SLI                                                                                                 | Recommended SLO                                      |
|--------------------------------------------|-----------------------------------------------------------------------------------------------------|------------------------------------------------------|
| `karmada-apiserver-availability`           | API server request success rate, counting HTTP 5xx and 429 responses and excluding health endpoints | 99.9% availability                                   |
| `karmada-apiserver-latency`                | API server request latency                                                                          | 99.9% of requests complete within 400ms              |
| `policy-apply-availability`                | Policy evaluation / policy matching success rate                                                    | 99.9% availability                                   |
| `policy-apply-latency`                     | Policy evaluation latency                                                                           | 99.9% of operations complete within 1.024s           |
| `karmada-scheduler-availability`           | Scheduler success rate for placement decisions                                                      | 99.9% availability                                   |
| `karmada-scheduler-latency`                | Scheduling latency                                                                                  | 99.9% of scheduling operations complete within 512ms |
| `karmada-scheduler-estimator-availability` | Scheduler estimator request success rate                                                            | 99.9% availability                                   |
| `karmada-scheduler-estimator-latency`      | Scheduler estimator response latency                                                                | 99.9% of estimations complete within 128ms           |
| `binding-sync-work-availability`           | Success rate of converting bindings into `Work` objects                                             | 99.9% availability                                   |
| `binding-sync-work-latency`                | Latency of binding-to-`Work` synchronization                                                        | 99.9% of operations complete within 1.024s           |
| `work-sync-workload-availability`          | Success rate of syncing `Work` payloads to member clusters                                          | 99.9% availability                                   |
| `work-sync-workload-latency`               | Latency of workload sync to member clusters                                                         | 99% of operations complete within 2.048s             |
| `cluster-sync-latency`                     | Time to synchronize cluster health/status information                                               | 99.9% of operations complete within 1s               |

In addition to the SLO targets themselves, Karmada now documents a complete operational model around them, including multi-window multi-burn-rate alerting, Sloth-based rule generation for Prometheus, Grafana SLO dashboards, and per-SLO runbooks for investigation and remediation.
...
Karmada provides and uses three main dashboard options:
...
- **SLO Dashboards**: The new [Reliability Engineering Guide](https://karmada.io/docs/administrator/reliability/guide) documents ready-to-use Grafana SLO dashboards based on Sloth-generated Prometheus rules, including an **SLO Details** dashboard for per-SLO drill-down and an **SLO Overview** dashboard for fleet-wide burn-rate and error-budget monitoring.

**Requirements**:
Karmada Dashboard requires deployment of the separate dashboard project. The Grafana dashboards require a monitoring stack capable of collecting Karmada metrics, typically Prometheus for scraping and storage, and Grafana for visualization. For the SLO dashboards, operators also need to load the Sloth-generated Prometheus recording and alerting rules described in the reliability guide.
...
Karmada now defines SLOs in the [Reliability Engineering Guide](https://karmada.io/docs/administrator/reliability/guide), where they are organized around the major stages of the control-plane workflow rather than only around a few top-level latency metrics. In addition, the community's 2022 [large-scale test report](https://karmada.io/blog/2022/10/26/test-report#slisslos) remains a useful reference for earlier large-scale SLI/SLO and performance validation.

The current documented SLOs are:

- **API server**
  - `karmada-apiserver-availability`: **99.9% availability**
  - `karmada-apiserver-latency`: **99.9% of requests within 400ms**
- **Policy application**
  - `policy-apply-availability`: **99.9% availability**
  - `policy-apply-latency`: **99.9% of operations within 1.024s**
- **Scheduling**
  - `karmada-scheduler-availability`: **99.9% availability**
  - `karmada-scheduler-latency`: **99.9% of operations within 512ms**
  - `karmada-scheduler-estimator-availability`: **99.9% availability**
  - `karmada-scheduler-estimator-latency`: **99.9% of estimations within 128ms**
- **Resource propagation**
  - `binding-sync-work-availability`: **99.9% availability**
  - `binding-sync-work-latency`: **99.9% of operations within 1.024s**
  - `work-sync-workload-availability`: **99.9% availability**
  - `work-sync-workload-latency`: **99% of operations within 2.048s**
- **Cluster health synchronization**
  - `cluster-sync-latency`: **99.9% of operations within 1 second**

These targets are documented as recommended defaults for adopters. The guide also explains the associated error-budget model, alerting strategy, Prometheus/Sloth setup flow, and runbooks so operators can adapt the objectives to their own environments and operationalize them in practice.

#### What are the SLIs (Service Level Indicators) an operator can use to determine the health of the service?

Operators can use the SLIs documented in the [Reliability Engineering Guide](https://karmada.io/docs/administrator/reliability/guide) to assess both the correctness and the responsiveness of the Karmada control plane.

The key SLIs are:

- **API server availability SLI**: success rate of `karmada-apiserver` requests, with HTTP 5xx and 429 responses treated as failures and health endpoints excluded from the calculation.
- **API server latency SLI**: request latency observed at `karmada-apiserver`.
- **Policy application availability/latency SLIs**: success rate and processing latency for policy matching/evaluation.
- **Scheduler availability/latency SLIs**: success rate and latency for multi-cluster placement decisions.
- **Scheduler estimator availability/latency SLIs**: success rate and response latency for estimator calls used during capacity-aware scheduling.
- **Binding sync availability/latency SLIs**: success rate and latency for converting `ResourceBinding`/`ClusterResourceBinding` objects into `Work` objects.
- **Work sync availability/latency SLIs**: success rate and latency for pushing `Work` payloads to member clusters.
- **Cluster sync latency SLI**: latency for collecting and synchronizing cluster health/status back to the control plane.

These SLIs are derived from Karmada's measurable metrics and are designed to cover the full propagation path from API entry, to policy selection, to scheduling, to final workload delivery in member clusters. Karmada also documents how to turn these SLIs into Prometheus recording rules, burn-rate alerts, and runbook-driven operations.
